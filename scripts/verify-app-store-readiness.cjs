...
check('Workflow iOS vérifie les profils Apple exacts', contains(iosWorkflow, 'profiles?filter[profileType]=IOS_APP_STORE&filter[bundleId]=') && contains(iosWorkflow, '&include=bundleId&limit=50') && contains(iosWorkflow, 'profile.relationships?.bundleId?.data?.id') && contains(iosWorkflow, "state === 'ACTIVE'") && contains(iosWorkflow, 'Apple App Store provisioning profile missing for'));
