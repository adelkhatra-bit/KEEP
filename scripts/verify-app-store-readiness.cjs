...
check('Workflow iOS vérifie les profils Apple exacts', contains(iosWorkflow, 'profiles?filter[profileType]=IOS_APP_STORE&include=bundleId&limit=200') && contains(iosWorkflow, 'includedBundlesById') && contains(iosWorkflow, 'profile.relationships?.bundleId?.data?.id') && contains(iosWorkflow, "state === 'ACTIVE'") && contains(iosWorkflow, 'Apple App Store provisioning profile missing for'));
