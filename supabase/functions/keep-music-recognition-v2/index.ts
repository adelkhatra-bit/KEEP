...
async function recognize(req: Request) {
  ...
  if (!credential) {
    ...
    return json(409, {
      ...
      message: "Reconnaissance musicale indisponible : remplace la clé AudD dans le Super Admin Loki Music.",
    });
  }
  ...
  try {
    ...
    return json(503, { error: "recognition_network_error", message: "Loki Music n’arrive pas à joindre le moteur musical. L’écoute reste active." });
  }
  ...
  if (!response.ok || payload?.status === "error") {
    ...
    if (isAuthorizationFailure(providerMessage)) {
      ...
      return json(409, { error: "recognition_not_configured", message: "La clé AudD enregistrée n’est pas valide. Remplace-la dans le Super Admin Loki Music." });
    }
    await setRuntimeStatus("ERROR", providerMessage);
    return json(502, { error: "recognition_provider_error", message: "Le moteur de reconnaissance a rencontré une erreur. Loki Music va réessayer automatiquement." });
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json(500, { error: "recognition_gateway_error", message: "Reconnaissance temporairement indisponible. Loki Music va réessayer automatiquement." });
  }
});
