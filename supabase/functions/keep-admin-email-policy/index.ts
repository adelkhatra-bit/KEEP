  ...
  try {
    ...
    if (action === "set") {
      ...
      const { error } = await admin.from("remote_config").upsert({
        ...
        value: enabled,
        description: "Activation globale depuis le Super Admin : demande une adresse e-mail vérifiée aux comptes Loki Music existants sans supprimer ni recréer leur profil.",
