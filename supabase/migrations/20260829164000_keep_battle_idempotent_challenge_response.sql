...
begin
 ...
 if c.status='ACCEPTED' and p_accept and c.arena_id is not null then
   ...
   return jsonb_build_object('id',c.id,'status','ACCEPTED','arenaId',c.arena_id,'arenaCode',acode,'themeCode',c.theme_code,'replayed',true);
 end if;
 ...
 return jsonb_build_object('id',c.id,'status','ACCEPTED','arenaId',aid,'arenaCode',acode,'themeCode',c.theme_code);
end;$function$;
