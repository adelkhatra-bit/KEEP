-- Adel (02/09/2026) : "je suis allé sur mon profil pour me remettre
-- disponible, ça m'avait désactivé" -- accepter un défi (keep_battle_challenge_respond)
-- et quitter le mode solo (keep_battle_solo_leave) supprimaient TOUTE la
-- ligne keep_battle_solo_presence, y compris manual_available -- la bascule
-- "disponible pour un Battle" est une préférence durable, indépendante du
-- fait de rejoindre UN match précis ; elle ne doit jamais être effacée en
-- silence par autre chose que l'utilisateur lui-même ou le TTL de 30 min.
-- On downgrade le statut au lieu de supprimer la ligne.
create or replace function public.keep_battle_solo_leave()
returns void
language plpgsql
security definer
...
begin
  ...
  update public.keep_battle_solo_presence set status = 'SOLO' where profile_id = auth.uid();
end;
$function$;
