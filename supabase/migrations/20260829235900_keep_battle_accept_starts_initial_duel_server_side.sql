-- Start the initial accepted duel on the server so both clients share one authoritative clock.
-- Existing-group invitations remain WAITING so more players can join between 8-round matches.
create or replace function public.keep_battle_challenge_respond(p_challenge_id uuid, p_accept boolean)
returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare uid uuid:=auth.uid(); c public.keep_battle_challenges%rowtype; created jsonb; aid uuid; acode text; min_free integer:=3; my_name text; arena_state jsonb; a public.keep_battle_arenas%rowtype; active_count integer;
begin
 ...
 arena_state:=public.keep_battle_arena_start(aid);
 return jsonb_build_object('id',c.id,'status','ACCEPTED','arenaId',aid,'arenaCode',acode,'themeCode',c.theme_code,'arenaState',arena_state,'serverStarted',true);
end;$function$;
