-- Adel (04/09/2026) : "j'ai envoye des invites et on voit pas le troisieme
-- joueur sur la jauge" -- BUG REEL confirme en base (deux challenges acceptes
-- a 300ms d'ecart avaient chacun leur PROPRE arena_id -- deux matchs 1 contre
-- 1 distincts crees depuis "Joueurs disponibles", jamais un seul match a
-- trois). Cause racine n°2, plus subtile : meme en regroupant les invites
-- dans la MEME arene (voir migration suivante côté client), le match
-- demarrait tout seul 1,8s apres l'arrivee du 2e joueur (keep_battle_arena_
-- start), sans jamais laisser le temps a un 3e invite encore en attente de
-- reponse d'accepter. keep_battle_arena_start refuse desormais de demarrer
-- tant qu'il reste une invitation PENDING pour cette arene, et
-- keep_battle_arena_state expose ce compte pour que le client sache quand
-- reessayer.
...
as $function$
declare uid uuid:=auth.uid(); a public.keep_battle_arenas%rowtype; active_count integer; round_start timestamptz; pending_count integer;
begin
  ...
  select count(*) into pending_count from public.keep_battle_challenges where arena_id=a.id and status='PENDING' and expires_at>now();
