-- KEEP — bibliothèque propriétaire complète pour l'anti-doublon.
--
-- Cette fonction est accessible uniquement au compte authentifié lui-même.
-- Elle renvoie ses KEEP PUBLIC + PRIVATE afin que le Swipe social puisse retirer
-- avant lecture tout morceau déjà possédé. L'écran Profil continue de filtrer
-- explicitement visibility='PUBLIC' avant affichage, donc aucune donnée privée
-- n'est exposée à un visiteur.
...
as $$
  ...
  select decision_id, kept_at, visibility, track_id, isrc, title, artist, album,
    duration_sec, artwork_url, genres, provider_ids, preview_url, external_urls,
    available_on, context, source_user_id, source_type
  from latest
