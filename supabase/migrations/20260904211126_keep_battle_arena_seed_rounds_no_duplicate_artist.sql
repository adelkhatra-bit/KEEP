...
-- dans le même Battle ou le même Artiste" -- BUG RÉEL : la sélection des
-- morceaux d'un Battle (candidates) ne dédoublonnait jamais par artiste,
-- seulement par piste (chaque ligne de public.tracks étant déjà unique par
-- construction). Deux morceaux différents du même artiste (ou deux versions
-- de la même chanson stockées comme deux lignes distinctes) pouvaient donc
-- tomber dans les mêmes round_count manches d'un match. Ajoute un DISTINCT
-- ON (artiste) avant le tirage final, en conservant exactement le même
-- tirage pseudo-aléatoire (md5 par arène+match+thème) comme départage intra-
-- artiste ET comme ordre final -- aucun autre comportement changé.
