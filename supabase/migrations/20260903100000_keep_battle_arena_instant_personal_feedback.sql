-- Adel (03/09/2026) : "je clique la mauvaise réponse mais ça me donne pas la
-- bonne réponse en vert ... utilise bien : mauvaise réponse en rouge, bonne
-- réponse en vert, la musique continue jusqu'à la prochaine" -- root-causé :
-- l'essai précédent (côté client uniquement) supposait que
-- keep_battle_arena_state exposait déjà `myAnswer.correct` et `round.artist`
-- dès que JE réponds. En réalité les deux restent NULL tant que la manche
-- entière n'est pas "finalized" (keep_battle_arena_finalize_round refuse de
-- s'exécuter tant que TOUS les joueurs actifs n'ont pas répondu OU que le
-- chrono n'est pas écoulé) -- sur un duel à 2, si mon adversaire n'a pas
-- encore répondu, rien n'était calculé pour moi, `correct` était `null`, et
-- le code client traitait ce `null` comme "faux" (d'où le rouge qui
-- s'allumait même sur un bon choix, jamais de vert nulle part).
--
-- Correctif ciblé, uniquement dans ce qui EST renvoyé à MOI pour MA propre
-- réponse -- ne touche à aucune logique de score/points (toujours calculés
-- une seule fois, par finalize_round, avec le bonus de rapidité inchangé) :
-- `myAnswer.correct` compare directement ma réponse à l'artiste déjà connu
-- en base (r.artist_snapshot existe dès le seed, seule sa diffusion dans le
-- JSON était conditionnée à finalized_at) ; `round.artist`/titre/pochette
-- me sont révélés dès que MOI j'ai répondu (pas besoin d'attendre les
-- autres) -- je ne peux plus changer ma réponse de toute façon, donc ça ne
-- crée aucune possibilité de tricher, juste le même retour immédiat que le
-- solo.
