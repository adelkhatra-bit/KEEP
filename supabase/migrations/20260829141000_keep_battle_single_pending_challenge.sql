with ranked as (
  select id,
         row_number() over (partition by challenger_id, target_id order by created_at desc, id desc) as rn
...
create or replace function public.keep_battle_challenge_send(p_target_id uuid,p_theme_code text default 'MIX') returns jsonb
