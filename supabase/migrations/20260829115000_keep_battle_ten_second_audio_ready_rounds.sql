update public.remote_config set value='10000'::jsonb where key='battle_arena_round_duration_ms';
alter table public.keep_battle_arenas alter column round_duration_ms set default 10000;
update public.keep_battle_arenas set round_duration_ms=10000 where status in ('WAITING','ACTIVE');
...
begin
 ...
 if elapsed>10000 then perform public.keep_battle_arena_finalize_round(a.id); return public.keep_battle_arena_state(a.id); end if;
