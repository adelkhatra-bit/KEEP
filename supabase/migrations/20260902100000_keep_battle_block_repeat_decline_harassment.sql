-- Adel (02/09/2026) : "pour éviter les abus au niveau des Battles pour les
-- invite, peut-être au bout de deux refus, empêcher l'utilisateur de
-- renvoyer une invite ... pour éviter les gens qui harcèlent" -- après 2
-- refus de la MÊME cible, ce challenger précis ne peut plus lui renvoyer de
-- défi (1v1 ou invitation de groupe). Ne bloque que ce couple précis
-- (challenger, cible) -- n'affecte jamais les autres joueurs.
...
begin
 ...
 select count(*) into decline_count from public.keep_battle_challenges where challenger_id=uid and target_id=p_target_id and status='DECLINED';
