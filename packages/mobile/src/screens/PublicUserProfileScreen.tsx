              ...
              return <View key={track.id} style={styles.musicRow}>
                ...
                <View style={styles.trackInfo}>
                  <View style={styles.trackTitleRow}>
                    ...
                    {/* Adel (08/09/2026) : "le bouton ajouter ... au-dessus,
                        le bouton pour la musique ... en dessous, de la meme
                        taille" -- pile fixe Jouer/Garder, toujours la meme
                        largeur, pour que le reste (Partager + coeur) ne
                        contienne plus jamais un texte assez long pour faire
                        sauter le coeur a la ligne de façon incoherente. */}
                    <View style={styles.trackPrimaryActions}>
                      <TrackPreviewButton trackKey={track.trackId} previewUrl={track.previewUrl} compact fullWidth />
                      {viewer?.id !== profile.id ? (
                        <TouchableOpacity style={[styles.keepButtonStacked, alreadyKept && styles.alreadyKeepButton]} onPress={() => alreadyKept ? showAlreadyKept(track.title) : void addToMyKeep(track)} disabled={adding}>
