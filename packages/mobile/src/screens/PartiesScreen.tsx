...
import { broadcastEventToFollowers, createCreatorEvent, loadMyRsvps, loadUpcomingEvents, setEventRsvp, CreatorEvent, EventRsvpStatus, loadMyPendingEventReviews, submitEventReview, loadEventReviewSummary, PendingEventReview, EventReviewSummary, loadEventRsvpCounts, EventRsvpCounts } from '../services/creatorEventService';
...
export default function PartiesScreen({ navigation, route }: any) {
  ...
  const publish = async (notifyFollowers: boolean) => {
    ...
    setCreateBusy(true);
    try {
      const created = await createCreatorEvent({ name: name.trim(), description: description.trim(), venueName: venueName.trim(), startsAt: iso, countryCode: countryCode.trim().toUpperCase().slice(0,2), djArtistNames: user?.username ? [user.username] : [], youtubeUrl: youtubeUrl.trim() || undefined, lat: venueCoords?.lat, lng: venueCoords?.lng });
      let sent = 0; if (notifyFollowers) sent = await broadcastEventToFollowers(created.id, message.trim(), includeRsvpButtons);
      setCreateOpen(false); setName(''); setStartsAt(''); setVenueName(''); setVenueCoords(null); setVenueSuggestions([]); setYoutubeUrl(''); setDescription(''); setMessage(''); setIncludeRsvpButtons(true);
      await reload();
  ...
  return <SafeAreaView style={styles.container}>
    ...
    <Modal visible={createOpen} transparent animationType="slide" onRequestClose={()=>setCreateOpen(false)}><View style={styles.backdrop}><View style={styles.sheet}><View style={styles.modalHeader}><Text style={styles.modalTitle}>Créer une soirée</Text><TouchableOpacity onPress={()=>setCreateOpen(false)}><Text style={styles.close}>Fermer</Text></TouchableOpacity></View><ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <TextInput style={styles.input} value={venueName} onChangeText={(v) => { setVenueName(v); setVenueCoords(null); }} placeholder="Lieu (tape une adresse…)" placeholderTextColor={colors.textMuted}/>
      {venueSearchBusy ? <ActivityIndicator color="#8B5CF6" style={styles.venueSuggestionsLoading}/> : venueSuggestions.length ? <View style={styles.venueSuggestions}>{venueSuggestions.map((s, i) => <TouchableOpacity key={`${s.lat}-${s.lng}-${i}`} style={styles.venueSuggestionRow} onPress={() => pickVenueSuggestion(s)}><Text style={styles.venueSuggestionText} numberOfLines={2}>{s.label}</Text></TouchableOpacity>)}</View> : null}
      ...
      <TouchableOpacity style={styles.rsvpToggleRow} onPress={() => setIncludeRsvpButtons((v) => !v)} accessibilityRole="checkbox" accessibilityState={{ checked: includeRsvpButtons }}>
