...
export default function PartiesScreen({ navigation, route }: any) {
  ...
  // photos" -- jusqu'a 3, ajoutees une a une.
  ...
  // Adel (17-18/09/2026) : "construis tout ce qui manque ... entrée
  // payante" -- null = gratuit, sinon un montant fixe de la liste.
  const [ticketPriceCents, setTicketPriceCents] = useState<number | null>(null);
  ...
  const confirmTicketPaid = async (order: EventTicketTransaction) => {
    ...
    setTicketConfirmBusyId(order.id);
    try {
      await markEventTicketPaid(order.id);
      setTicketSales((rows) => rows.map((r) => r.id === order.id ? { ...r, status: 'COMPLETED' } : r));
    } catch {
      Alert.alert('Confirmation', 'Impossible de confirmer ce paiement pour le moment.');
    } finally {
      setTicketConfirmBusyId(null);
    }
  };
  ...
  // Adel (17-18/09/2026) : "construis tout ce qui manque ... entrée
  // payante ... trouve une solution" -- même modèle que le marketplace :
  // lien de paiement personnel de l'organisateur, KEEP n'encaisse rien.
  // Un évènement payant ne débloque jamais "GOING" directement -- ça passe
  // par une demande d'achat, puis la confirmation manuelle de
  // l'organisateur (keep_event_ticket_mark_paid) fait le vrai upsert RSVP.
  const buyEventTicket = async (event: CreatorEvent) => {
    if (busyId) return;
    setBusyId(event.id);
    try {
  ...
  const chooseRsvp = async (eventId: string, status: EventRsvpStatus, advanceAfter = false) => {
    ...
    const targetEvent = events.find((e) => e.id === eventId) ?? (currentEvent?.id === eventId ? currentEvent : null);
    if (status === 'GOING' && targetEvent?.ticketPriceCents && targetEvent.creatorId !== user.id && rsvps[eventId] !== 'GOING') {
      void buyEventTicket(targetEvent);
      return;
    }
    setBusyId(eventId);
    try {
      ...
      if (status === 'NOT_GOING') {
        // Adel (08/09/2026) : "il pourra effacer l'evenement et il n'aura
        // pas de rappel de l'evenement" -- retire l'evenement de la liste
        // locale ; côté serveur, les rappels jour-J excluent deja NOT_GOING
        // (keep_event_send_day_of_reminders ne cible que GOING/MAYBE).
  ...
  const pickEventImage = async () => {
    if (!user || imageUploadBusy || eventImageUrls.length >= 3) return;
  ...
  // texte ... il faut pas que les utilisateurs voient quoi que ce soit tant
  // que le super admin a pas approuve" -- la diffusion ne part plus a la
  // creation : elle part automatiquement (cote serveur, admin_event_approve)
  // au moment ou le Super Admin valide photo+texte. Modifier reste
  // silencieux (re-notifier a chaque correction serait du spam), et de
  ...
  // Adel (08/09/2026) : "celui qui fait l'invite pourquoi il reçoit une
  // invite comme si ... il puisse voir comment les utilisateurs le verront,
  // mais je veux pas qu'il ait la même config" -- l'organisateur voit sa
  ...
  return <SafeAreaView style={styles.container}>
    ...
    <Modal visible={createOpen} transparent animationType="slide" onRequestClose={resetEventForm}><View style={styles.backdrop}><View style={styles.sheet}><View style={styles.modalHeader}><Text style={styles.modalTitle}>{editingEventId ? 'Modifier l’événement' : 'Créer un événement'}</Text><TouchableOpacity onPress={resetEventForm}><Text style={styles.close}>Fermer</Text></TouchableOpacity></View><ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      {/* Adel (08/09/2026) : "le truc photo tu le remontes un peu plus
          ...
          d'ajout tant qu'il reste de la place. */}
      <View style={styles.photoGalleryRow}>
        {eventImageUrls.map((url) => (
          <View key={url} style={styles.photoGalleryThumbWrap}>
            <Image source={{ uri: url }} style={styles.photoGalleryThumb} resizeMode="cover" />
            <TouchableOpacity style={styles.photoGalleryRemove} onPress={() => removeEventImage(url)} accessibilityLabel="Retirer cette photo">
              <Text style={styles.photoGalleryRemoveText}>✕</Text>
            </TouchableOpacity>
          </View>
        ))}
        {eventImageUrls.length < 3 ? (
          <TouchableOpacity style={styles.photoGalleryAdd} onPress={() => void pickEventImage()} disabled={imageUploadBusy}>
            {imageUploadBusy ? <ActivityIndicator color={colors.primaryLight}/> : <Text style={styles.photoGalleryAddText}>📷{'\n'}Ajouter</Text>}
          </TouchableOpacity>
        ) : null}
      </View>
      ...
      {/* Adel (17-18/09/2026) : "construis tout ce qui manque ... entrée
          ...
          personnel, KEEP n'encaisse rien, prix en montants fixes. */}
      <Text style={styles.rsvpToggleLabel}>Entrée payante ?</Text>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 6, marginBottom: 10 }}>
        <TouchableOpacity style={[styles.eventPriceChip, ticketPriceCents === null && styles.eventPriceChipOn]} onPress={() => setTicketPriceCents(null)}>
          <Text style={[styles.eventPriceChipText, ticketPriceCents === null && styles.eventPriceChipTextOn]}>Gratuit</Text>
        </TouchableOpacity>
        {EVENT_TICKET_PRESET_PRICES_CENTS.map((cents) => (
          <TouchableOpacity key={cents} style={[styles.eventPriceChip, ticketPriceCents === cents && styles.eventPriceChipOn]} onPress={() => setTicketPriceCents(cents)}>
            <Text style={[styles.eventPriceChipText, ticketPriceCents === cents && styles.eventPriceChipTextOn]}>{(cents / 100).toFixed(2)}€</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {/* Adel (08/09/2026) : "les trucs à cocher tu les mets ensemble en
          ...
          reglages a cocher regroupes juste avant de publier, plutot que
          disperses dans le formulaire (ce qui donnait l'impression de
          textes en double). */}
    ...
    <Modal visible={participantsOpen} transparent animationType="slide" onRequestClose={() => setParticipantsOpen(false)}>
      <View style={styles.backdrop}><View style={styles.sheet}>
        ...
        {/* Adel (08/09/2026) : "il y a un code, je sais pas exactement à
            quoi ça sert" -- pas de camera pour l'instant : chaque
            participant voit son propre code sous son QR (bouton 🎟 Mon
            billet) ; s'il ne peut pas scanner, il te le dicte et tu le
            tapes ici. Le bouton "Présent ?" en face de son nom marche sans
    ...
    <Modal visible={eventDetailOpen && !!currentEvent} transparent animationType="slide" onRequestClose={() => setEventDetailOpen(false)}>
      <View style={styles.backdrop}><View style={styles.sheet}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          {currentEvent && currentEvent.imageUrls.length ? (
            <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={styles.detailImageScroll}>
              {currentEvent.imageUrls.map((url) => <Image key={url} source={{ uri: url }} style={styles.detailImage} resizeMode="contain" />)}
            </ScrollView>
          ) : null}
          <Text style={styles.date}>{dateText}</Text>
          <Text style={styles.meta}>{currentEvent ? [currentEvent.venueName,currentEvent.countryCode].filter(Boolean).join(' · ') : ''}</Text>
          {currentEvent?.description ? <Text style={styles.detailDescription}>{currentEvent.description}</Text> : null}
          {currentEvent && !isOwnEvent ? <View style={styles.rsvpMainRow}>
            <TouchableOpacity style={[styles.rsvpButton,styles.rsvpButtonNo]} disabled={busyId===currentEvent.id} onPress={()=>{setEventDetailOpen(false); void chooseRsvp(currentEvent.id,'NOT_GOING',true);}}><Text style={styles.rsvpButtonNoText}>✕ Je ne participe pas</Text></TouchableOpacity>
            <TouchableOpacity style={[styles.rsvpButton,styles.rsvpButtonYes]} disabled={busyId===currentEvent.id} onPress={()=>{setEventDetailOpen(false); void chooseRsvp(currentEvent.id,'GOING',true);}}><Text style={styles.rsvpButtonYesText}>{currentEvent.ticketPriceCents && rsvps[currentEvent.id] !== 'GOING' ? `🎟 Payer ${(currentEvent.ticketPriceCents / 100).toFixed(2)}€` : '✓ Je participe'}</Text></TouchableOpacity>
          </View> : null}
        </ScrollView>
      </View></View>
    </Modal>
