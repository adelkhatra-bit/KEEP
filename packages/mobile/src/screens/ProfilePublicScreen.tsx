...
import { KeepVisibility, ProfileKind, SocialLink } from '../types';
...
export default function ProfilePublicScreen({ navigation }: any) {
  ...
  const ownKeepCount = useMemo(() => keptTracks.filter((entry) => entry.creditSource !== 'SOCIAL' && !entry.sourceProfileId && !entry.sourceUsername).length, [keptTracks]);
  const userKeepCount = useMemo(() => keptTracks.filter((entry) => entry.creditSource === 'SOCIAL' || !!entry.sourceProfileId || !!entry.sourceUsername).length, [keptTracks]);
  ...
  const renderCompactTrack = (track: CanonicalTrack, key: string, sourceUsername?: string | null, visibility?: KeepVisibility) => (
    <View key={key} style={s.keepRow}>
      ...
      <View style={s.keepInfo}>
        ...
        <View style={s.trackMetaRow}>
          ...
          {visibility ? <View style={[s.visibilityBadge, visibility === 'PUBLIC' ? s.visibilityPublic : s.visibilityPrivate]}><Text style={s.visibilityText}>{visibility === 'PUBLIC' ? '👁 PUBLIC' : '🔒 PRIVÉ'}</Text></View> : null}
          {sourceUsername ? (
  ...
  const tabContent = () => {
    if (activeTab === 'KEEP') {
      ...
      return <View style={s.keepList}>
        <Text style={s.ownerKeepHint}>Tu vois ici toute ta bibliothèque KEEP. Les morceaux 🔒 Privé restent invisibles pour les visiteurs ; seuls les morceaux 👁 Public apparaissent sur ton profil partagé.</Text>
        {keptTracks.map((entry) => renderCompactTrack(entry.track, entry.id, entry.sourceUsername ?? null, entry.visibility ?? 'PRIVATE'))}
      </View>;
    }
