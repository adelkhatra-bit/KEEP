...
export default function AccountEmailPanel({ enabled, username }: { enabled: boolean; username: string }) {
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      setCode('');
      await refresh();
      Alert.alert('Code envoyé', `Loki Music a envoyé un code à ${result.emailHint}. Il expire dans 10 minutes.`);
    } catch (e: any) {
  ...
  const confirmCode = async () => {
    ...
    try {
      ...
      setCodeSent(false);
      setCode('');
      Alert.alert('Adresse validée', `Tu peux maintenant te connecter à Loki Music avec ${username} ou ${next.email}.`);
    } catch (e: any) {
...
const s = StyleSheet.create({
  ...
  help: { color: colors.textMuted, fontSize: 14, lineHeight: 16 },
  ...
  pending: { color: colors.primaryLight, fontSize: 14, fontWeight: '800', marginBottom: 7 },
  ...
  secondaryText: { color: colors.primaryLight, fontSize: 14, fontWeight: '900' },
});
