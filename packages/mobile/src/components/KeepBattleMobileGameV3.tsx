...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  if (arena) {
    ...
    return <View style={s.root}>
      ...
      {/* Adel (04/09/2026) : "on pourra intégrer le nom de la personne" --
          au-delà de 2 joueurs, plus de jauge à 2 côtés arbitraire : un
          mini-classement avec chaque vrai nom, trié par score, le
          meneur actuel mis en avant. */}
      {players.length > 2 ? <View style={s.groupStandings}><Text style={s.groupStandingsTitle}>{players.length} JOUEURS · {arena.status === 'ACTIVE' ? `${Math.ceil(left / 1000)}s` : 'PRÊT'}</Text>{players.map((player, index) => <TouchableOpacity key={player.profileId} style={[s.groupStandingRow, index === 0 && s.groupStandingRowLead]} onPress={() => onOpenProfile(player.username)}><Text style={s.groupStandingRank}>{index === 0 ? '👑' : `#${index + 1}`}</Text><Text style={s.groupStandingName} numberOfLines={1}>@{player.username}</Text><Text style={s.groupStandingScore}>{Number(player?.score || 0)} pts</Text></TouchableOpacity>)}</View> : null}
