...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  const insufficientForOpponent = React.useCallback((player: KeepBattleLivePlayer) => (
    player.hasPaidAccess !== true && player.remainingFree < stakeForRounds(roundCount)
  ), [roundCount]);
  ...
  if (solo) {
    ...
    return <View style={s.root}>
      ...
      <View style={s.matchContextBar}><Text style={s.matchContextMode}>SOLO</Text><Text style={s.matchContextText}>{soloIndex + 1} / {solo.rounds.length} · {themeLabel(solo.rounds[soloIndex]?.themeCode || solo.themeCode)}</Text><Text style={s.matchContextStake}>🎁 +{maxRewardForRounds(solo.rounds.length)}</Text></View>
  ...
  if (arena) {
    ...
    return <View style={s.root}>
      ...
      <View style={s.matchContextBar}><Text style={s.matchContextMode}>EN LIGNE</Text><Text style={s.matchContextText}>{arena.seats.length} joueur{arena.seats.length > 1 ? 's' : ''} · manche {arena.currentRound || 0}/{arena.roundCount}</Text><Text style={s.matchContextStake}>🎁 {stakeForRounds(arena.roundCount)}</Text></View>
  ...
  return <View style={s.root}><View style={s.home}><TouchableOpacity style={s.homeBack} onPress={onExit}><Text style={s.homeBackText}>‹</Text></TouchableOpacity>
      ...
      ) : null}{!incoming[0] && pendingRematch[0] ? <Animated.View style={[s.invite, { transform: [{ scale: pulse }] }]}><View style={s.inviteHead}><View style={{ flex: 1 }}><Text style={s.inviteQuestion}>🔁 Revanche avec {pendingRematch[0].participantUsernames.map((u) => `${u}`).join(', ') || 'le groupe'}. On repart ?</Text><Text style={s.inviteLabel}>⚡ {themeLabel(pendingRematch[0].themeCode)} · {Math.max(0, Math.ceil((new Date(pendingRematch[0].rematchDeadline).getTime() - now) / 1000))}s pour répondre</Text></View></View><View style={s.inviteActions}><TouchableOpacity accessibilityRole="button" accessibilityLabel="Refuser la revanche" hitSlop={10} disabled={Boolean(rematchBannerBusyId)} style={[s.no, rematchBannerBusyId && s.actionDisabled]} onPress={() => { void respondPendingRematch(pendingRematch[0], false); }}><Text style={s.noText}>REFUSER</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="Accepter la revanche" hitSlop={10} disabled={Boolean(rematchBannerBusyId)} style={[s.yes, rematchBannerBusyId && s.actionDisabled]} onPress={() => { void respondPendingRematch(pendingRematch[0], true); }}><Text style={s.yesText}>{rematchBannerBusyId === pendingRematch[0].arenaId ? 'CONNEXION…' : 'ACCEPTER'}</Text></TouchableOpacity></View></Animated.View> : null}{incoming[0] ? <Animated.View style={[s.invite, { transform: [{ scale: pulse }] }]}><View style={s.inviteHead}><Avatar name={incoming[0].username} url={incoming[0].avatarUrl} size={48} /><View style={{ flex: 1 }}><Text style={s.inviteQuestion}><Text style={s.inviteName}>{incoming[0].username}</Text> te défie sur un Battle. Tu acceptes ?</Text><Text style={s.inviteLabel}>⚡ {themeLabel(incoming[0].themeCode)} · {incoming[0].roundCount} morceaux · {homeChallengeRemaining}s</Text></View></View>{respondingChallengeId === incoming[0].id ? <Text style={s.inviteConnecting}>CONNEXION AU BATTLE…</Text> : null}<View style={s.inviteActions}><TouchableOpacity accessibilityRole="button" accessibilityLabel="Refuser le Battle" hitSlop={10} disabled={Boolean(respondingChallengeId)} style={[s.no, respondingChallengeId && s.actionDisabled]} onPress={() => { void respond(incoming[0], false); }}><Text style={s.noText}>REFUSER</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="Accepter le Battle" hitSlop={10} disabled={Boolean(respondingChallengeId)} style={[s.yes, respondingChallengeId && s.actionDisabled]} onPress={() => { void respond(incoming[0], true); }}><Text style={s.yesText}>{respondingChallengeId === incoming[0].id ? 'CONNEXION…' : 'ACCEPTER'}</Text></TouchableOpacity></View></Animated.View> : null}<View style={s.battleSetupCard}>
        <View style={s.battleSetupHeader}><View><Text style={s.battleSetupEyebrow}>FORMAT DU BATTLE</Text><Text style={s.battleSetupTitle}>{roundCount} morceaux · {stakeForRounds(roundCount)} Free en ligne</Text></View><Text style={s.battleSetupBadge}>10 s / titre</Text></View>
        <Text style={s.section}>NOMBRE DE MORCEAUX</Text>
        <ScrollView horizontal style={s.themeScroll} showsHorizontalScrollIndicator={false} contentContainerStyle={s.themeRow}>{ROUND_COUNT_OPTIONS.map((n) => { const short = insufficientForRoundCount(n); return <TouchableOpacity key={n} onPress={() => setRoundCount(n)} style={[s.theme, n === roundCount && s.themeOn, short && s.themeShort]}><Text style={[s.themeText, n === roundCount && s.themeTextOn, short && s.themeTextShort]}>{n}</Text><Text style={[s.themeStake, n === roundCount && s.themeStakeOn, short && s.themeTextShort]}>🎁 {stakeForRounds(n)}</Text></TouchableOpacity>; })}</ScrollView>
        {insufficientForRoundCount(roundCount) ? <Text style={s.themeShortWarning}>Il faut {stakeForRounds(roundCount)} Free pour un Battle en ligne de {roundCount} morceaux. Solde actuel : {myCreditStatus?.remainingFree ?? 0}.</Text> : null}
      ...
      <View style={s.battleModes}>
        <TouchableOpacity style={s.mainButton} disabled={busy} onPress={() => { void startSolo(); }}>
          {busy ? <ActivityIndicator color="#FFFFFF" /> : <><View style={s.modeTopRow}><View style={s.modeIcon}><Text style={s.modeIconText}>◎</Text></View><View style={s.modeCopy}><Text style={s.modeEyebrow}>ENTRAÎNEMENT</Text><Text style={s.mainButtonText}>JOUER SOLO</Text></View><Text style={s.modeChevron}>›</Text></View><Text style={s.mainButtonSub}>Teste ton oreille, progresse et gagne jusqu’à {maxRewardForRounds(roundCount)} Free.</Text></>}
        </TouchableOpacity>
        <TouchableOpacity style={s.onlineButton} disabled={busy} onPress={() => { void openOnline(); }}>
          <View style={s.modeTopRow}><View style={[s.modeIcon,s.modeIconOnline]}><Text style={s.modeIconText}>⚡</Text></View><View style={s.modeCopy}><Text style={s.modeEyebrow}>MULTIJOUEUR</Text><Text style={s.onlineTitle}>BATTLE EN LIGNE</Text></View><Text style={s.modeChevron}>›</Text></View>
          <Text style={s.onlineSub}>Choisis 1 à 9 adversaires · 1 adversaire = duel · plusieurs = Battle de groupe.</Text>
        </TouchableOpacity>
      </View></View>;
}
...
const s = StyleSheet.create({
  ...
  matchContextBar: { minHeight: 40, marginBottom: 8, paddingHorizontal: 10, borderRadius: 14, backgroundColor: colors.backgroundElevated, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  modeEyebrow: { color: '#DCD5F8', fontSize: 11, fontWeight: '900', letterSpacing: 1.1, marginBottom: 2 },
  modeChevron: { color: '#FFFFFF', fontSize: 24, fontWeight: '800' },
  ...
  squareBadge: { alignSelf: 'flex-start', margin: 3, minWidth: 18, height: 16, paddingHorizontal: 4, borderRadius: 8, backgroundColor: 'rgba(0,0,0,.55)', alignItems: 'center', justifyContent: 'center' }, squareBadgeText: { color: colors.primaryLight, fontSize: 11, fontWeight: '900' },
  ...
  groupStandings: { marginBottom: 8, padding: 10, borderRadius: 16, backgroundColor: colors.backgroundCard, borderWidth: 1, borderColor: colors.border, gap: 6 }, groupStandingsTitle: { color: colors.primaryLight, fontSize: 11, fontWeight: '900', letterSpacing: .6, marginBottom: 3, textAlign: 'center' }, groupStandingRow: { minHeight: 36, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 10, borderRadius: 12, backgroundColor: colors.backgroundElevated }, groupStandingRowLead: { borderWidth: 1, borderColor: colors.primaryLight }, groupStandingRank: { width: 26, textAlign: 'center', fontSize: 13, fontWeight: '900', color: '#FFF' }, groupStandingName: { flex: 1, color: '#FFF', fontSize: 12, fontWeight: '900', textDecorationLine: 'underline' }, groupStandingScore: { color: colors.success, fontSize: 12, fontWeight: '900' }, groupStandingsMore: { color: colors.textMutedGrey, fontSize: 11, fontWeight: '800', textAlign: 'center', marginTop: 2 },
