  ...
  return (
    <View style={eqStyles.row}>
      {values.map((value, i) => (
        <Animated.View
          key={i}
          style={[eqStyles.bar, { height: value.interpolate({ inputRange: [0, 1], outputRange: [10, 60] }) }]}
        />
      ))}
    </View>
  );
...
const eqStyles = StyleSheet.create({
  ...
  bar: { width: 8, borderRadius: 4, backgroundColor: '#E5F266' },
});
