...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  // Adel (02/09/2026) : "il retourne bien au bon endroit mais il revient
  // toujours" -- × et ‹ n'ont PAS le même sens : × ("Fermer le Battle")
  // quitte tout à fait (closeBattleArena, onExit inclus, retour à Soirées).
  // ‹ ("retour") doit juste revenir à l'accueil INTERNE de Battle (écran
  // JOUER SOLO / BATTLE EN LIGNE) pour pouvoir relancer une partie tout de
  // suite, sans démonter tout le composant -- appeler onExit ici aurait
  // démonté l'écran vers la carte "Salon musical" au lieu de l'accueil
  // Battle attendu, ce qui ressemblait à "ça revient" une fois remonté.
  ...
  if (solo) {
    ...
    return <View style={s.root}>
      ...
      {enabled ? <View style={s.live}><View style={s.liveHeader}><View style={s.dot} /><Text style={s.liveTitle}>{livePlayers.length ? `${livePlayers.length} joueur${livePlayers.length > 1 ? 's' : ''} disponible${livePlayers.length > 1 ? 's' : ''}` : 'Tu es visible pour les Battles'}</Text></View>{livePlayers.length ? <View style={s.liveList}>{livePlayers.slice(0, 3).map((p) => <View key={p.profileId} style={s.liveRowCompact}><TouchableOpacity style={s.liveRowLeft} onPress={() => onOpenProfile(p.username)}><Avatar name={p.username} url={p.avatarUrl} size={32} /><Text numberOfLines={1} style={s.liveRowName}>@{p.username}{p.skillTier ? ` · ${tierLabel(p.skillTier)}` : ''}</Text></TouchableOpacity>{(() => { const sent = outgoingPendingTargetIds.has(p.profileId); return <TouchableOpacity disabled={Boolean(challengeBusyId) || sent} style={[s.battleButton, challengeBusyId === p.profileId && s.battleButtonSending, sent && s.battleButtonSent, challengeBusyId && challengeBusyId !== p.profileId && s.actionDisabled]} onPress={() => { void challenge(p); }}><Text style={[s.battleButtonText, sent && s.battleButtonSentText]}>{challengeBusyId === p.profileId ? 'ENVOI…' : sent ? 'ENVOYÉ ✓' : 'BATTLE'}</Text></TouchableOpacity>; })()}</View>)}</View> : null}</View> : null}
      </ScrollView>
    </View>;
  }
