...
import { ActivityIndicator, Animated, Image, Modal, ScrollView, Share, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  React.useEffect(() => {
    setThemeCode(myPreferredThemes.length === 1 ? myPreferredThemes[0] : 'MIX');
  }, [myPreferredThemes]);
  ...
  const renderTeamSquares = (
    ...
    // Adel (03/09/2026) : "on voit pas les titres en dessous, on voit pas la
    // suite du bas" -- vrai bug : les carrés (avatar 44 + nom + score, 84px
    // de haut chacun) poussaient la question/les 3 réponses hors écran sur
    // un vrai match, sans aucun scroll pour rattraper -- inacceptable pendant
    // un chrono de quelques secondes. Carrés bien plus petits, en ligne
    // (wrap), sans doublon de score (déjà affiché juste au-dessus dans
    // l'en-tête VS) : la grille tient sur une seule petite rangée pour un
    // 1 contre 1, et ne grandit que si l'équipe est vraiment nombreuse.
    <View style={s.squareGrid}>
      ...
      <View style={s.squareCol}>
        ...
        {plusTile ? <TouchableOpacity disabled={plusTile.busy} style={[s.squareTile, s.squarePlus]} onPress={plusTile.onPress}>
          {plusTile.busy ? <ActivityIndicator color="#E5F266" size="small" /> : <><Text style={s.squarePlusIcon}>+</Text><Text style={s.squareName}>Rejoindre</Text></>}
  ...
  if (arena) {
    ...
    return <View style={s.root}>
      ...
      <View style={s.answers}>{(round.choices || []).slice(0, 3).map((choice, i) => <TouchableOpacity key={choice} disabled={Boolean(!ready || round.answered || round.revealed || pending || left <= 0)} onPress={() => { void answerArena(choice); }} style={[s.answer, (round.myAnswer?.selectedAnswer === choice || pending === choice) && !round.myAnswer && s.answerSelected, Boolean(round.myAnswer) && choice === round.artist && s.answerCorrect, round.myAnswer?.selectedAnswer === choice && choice !== round.artist && s.answerWrong]}><Text style={s.answerNo}>{i + 1}</Text><Text numberOfLines={2} style={s.answerText}>{primaryArtistLabel(choice)}</Text>{choice === round.myAnswer?.selectedAnswer && round.myAnswer?.responseMs != null ? <Text style={s.answerTime}>{(round.myAnswer.responseMs / 1000).toFixed(1)}s</Text> : null}</TouchableOpacity>)}</View></Animated.View></> : null}
      </ScrollView>
    </View>;
  }
...
const s = StyleSheet.create({
  ...
  squareGrid: { flexDirection: 'row', gap: 8, marginTop: 8 }, squareCol: { flex: 1, flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, squareTile: { width: 60, height: 54, borderRadius: 12, borderWidth: 1, borderColor: '#30273A', backgroundColor: '#17121D', alignItems: 'center', justifyContent: 'center', paddingVertical: 4, gap: 2 }, squareName: { color: '#FFF', fontSize: 11, fontWeight: '900', maxWidth: '92%' }, squareScore: { color: '#E5F266', fontSize: 11, fontWeight: '900' }, squarePlus: { borderWidth: 1.5, borderStyle: 'dashed', borderColor: '#8B5CF6', backgroundColor: '#1B1422' }, squarePlusIcon: { color: '#8B5CF6', fontSize: 18, fontWeight: '900', lineHeight: 20 },
