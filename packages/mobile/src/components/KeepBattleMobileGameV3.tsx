...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  if (arena) {
    ...
    return <View style={s.root}>
      ...
      <View style={s.answers}>{(round.choices || []).slice(0, 3).map((choice, i) => <TouchableOpacity key={choice} disabled={Boolean(!ready || round.answered || round.revealed || pending || left <= 0)} onPress={() => { void answerArena(choice); }} style={[s.answer, (round.myAnswer?.selectedAnswer === choice || pending === choice) && !round.myAnswer && s.answerSelected, round.myAnswer?.selectedAnswer === choice && round.myAnswer?.correct && s.answerCorrect, round.myAnswer?.selectedAnswer === choice && round.myAnswer && !round.myAnswer.correct && s.answerWrong, round.revealed && choice === round.artist && s.answerCorrect]}><Text style={s.answerNo}>{i + 1}</Text><Text numberOfLines={2} style={s.answerText}>{primaryArtistLabel(choice)}</Text>{choice === round.myAnswer?.selectedAnswer && round.myAnswer?.responseMs != null ? <Text style={s.answerTime}>{(round.myAnswer.responseMs / 1000).toFixed(1)}s</Text> : null}</TouchableOpacity>)}</View></Animated.View></> : null}
    </View>;
  }
