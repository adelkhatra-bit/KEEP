...
import { buildKeepBattleArenaInviteLink, KeepBattleArenaState, KeepBattleArenaWinner, KeepBattleTheme, leaveKeepBattleArena, loadKeepBattleArena, loadKeepBattleArenaWinnerHistory, loadKeepBattleGlobalLeaderboard, loadKeepBattleThemes, proposeKeepBattleArenaRematch, respondKeepBattleArenaRematch, startKeepBattleArena, submitKeepBattleArenaQuizAnswer, subscribeKeepBattleArena } from '../services/keepBattleService';
...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  if (solo) {
    ...
    return <View style={s.root}>
      ...
      {enabled ? <View style={s.live}><View style={s.liveHeader}><View style={s.dot} /><Text style={s.liveTitle}>{livePlayers.length ? `${livePlayers.length} joueur${livePlayers.length > 1 ? 's' : ''} disponible${livePlayers.length > 1 ? 's' : ''}` : 'Tu es visible pour les Battles'}</Text></View>{livePlayers.length ? <View style={s.liveList}>{livePlayers.slice(0, 3).map((p) => <View key={p.profileId} style={s.liveRowCompact}><TouchableOpacity style={s.liveRowLeft} onPress={() => onOpenProfile(p.username)}><Avatar name={p.username} url={p.avatarUrl} size={32} /><Text numberOfLines={1} style={s.liveRowName}>@{p.username}</Text></TouchableOpacity><TouchableOpacity disabled={Boolean(challengeBusyId)} style={[s.battleButton, challengeBusyId === p.profileId && s.battleButtonSending, challengeBusyId && challengeBusyId !== p.profileId && s.actionDisabled]} onPress={() => { void challenge(p); }}><Text style={s.battleButtonText}>{challengeBusyId === p.profileId ? 'ENVOI…' : 'BATTLE'}</Text></TouchableOpacity></View>)}</View> : null}</View> : null}
    </View>;
  }
  ...
  if (arena) {
    ...
    if (arena.status === 'WAITING' && arena.lastResult) {
      ...
      return <View style={s.root}>
        ...
        <View style={s.header}><TouchableOpacity style={s.back} onPress={closeBattleArena}><Text style={s.backText}>‹</Text></TouchableOpacity><View style={s.headerMid}><Text style={s.kicker}>Loki BATTLE · FIN DU MATCH</Text><Text style={s.title}>{themeLabel(arena.themeCode)}</Text></View><Text style={s.round}>{arena.seats.length}J</Text></View>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.finishScroll}>
          <Animated.View style={[s.finishHero, { opacity: celebrationOpacity, transform: [{ scale: celebrationScale }] }]}>
            <Text style={s.finishSpark}>✦ 👑 ✦</Text>
    ...
    return <View style={s.root}>
      ...
      <View style={s.header}><TouchableOpacity style={s.back} onPress={closeBattleArena}><Text style={s.backText}>‹</Text></TouchableOpacity><View style={s.headerMid}><Text style={s.kicker}>Loki BATTLE · {arena.seats.length} JOUEURS</Text><Text style={s.title}>{themeLabel(arena.themeCode)}</Text></View><Text style={s.round}>{arena.currentRound || 0}/{arena.roundCount}</Text></View>
