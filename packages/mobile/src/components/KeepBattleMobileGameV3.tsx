  ...
  if (arena) {
    ...
    return <View style={s.root}>
      ...
      {/* Adel (05/09/2026) : "pourquoi il y a un vide sur le bouton" -- BUG
          ...
          rester à 48%. */}
      <View style={s.answers}>{(round.choices || []).slice(0, 3).map((choice, i) => <TouchableOpacity key={choice} disabled={Boolean(!ready || round.answered || round.revealed || pending || left <= 0)} onPress={() => { void answerArena(choice); }} style={[s.answer, i === 2 && s.answerFull, (round.myAnswer?.selectedAnswer === choice || pending === choice) && !round.myAnswer && s.answerSelected, Boolean(round.myAnswer) && choice === round.artist && s.answerCorrect, round.myAnswer?.selectedAnswer === choice && choice !== round.artist && s.answerWrong]}><Text style={s.answerNo}>{i + 1}</Text><Text numberOfLines={3} style={[s.answerText, i === 2 && s.answerTextCenter]}>{primaryArtistLabel(choice)}</Text>{choice === round.myAnswer?.selectedAnswer && round.myAnswer?.responseMs != null ? <Text style={s.answerTime}>{(round.myAnswer.responseMs / 1000).toFixed(1)}s</Text> : null}</TouchableOpacity>)}</View></Animated.View></> : null}
      </ScrollView>
    </View>;
  }
