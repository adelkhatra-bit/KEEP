...
export default function KeepBattleMobileGameV3({ enabled, onOpenProfile, onRequireAccount, onExit, initialArenaId, onOpenSession }: Props) {
  ...
  if (arena) {
    ...
    return <View style={s.root}>
      ...
      <ScrollView style={s.arenaScroll} showsVerticalScrollIndicator={false} contentContainerStyle={s.arenaScrollContent}>
      {arena.status === 'WAITING' ? <View style={s.waiting}><View style={s.waitingPulse}><Text style={s.trophy}>⚡</Text></View><Text style={s.winner}>{arena.seats.length < 2 ? (arena.pendingInviteCount > 0 ? 'INVITATION ENVOYÉE' : 'EN ATTENTE') : 'JOUEURS EN SYNCHRONISATION'}</Text><Text style={s.waitText}>{arena.seats.length >= 2 ? 'Tous les joueurs sont réunis. Le morceau démarre sur le même chrono.' : arena.pendingInviteCount > 0 ? `${arena.pendingInviteCount} réponse${arena.pendingInviteCount > 1 ? 's' : ''} en attente · tu peux continuer à inviter d’autres joueurs.` : 'Choisis un adversaire ou partage le lien pour lancer le Battle.'}</Text>{arena.pendingInviteCount > 0 ? <View style={s.waitingStatusPill}><Text style={s.waitingStatusText}>EN ATTENTE DE RÉPONSE</Text></View> : null}</View> : null}
  ...
  return <View style={s.root}><View style={s.home}><TouchableOpacity style={s.homeBack} onPress={onExit}><Text style={s.homeBackText}>‹</Text></TouchableOpacity>
      ...
      <View style={s.battleModes}>
        ...
        <TouchableOpacity style={s.onlineButton} disabled={busy} onPress={() => { void openOnline(); }}>
          ...
          <Text style={s.onlineSub}>Choisis 1 à 9 adversaires. Une sélection = un vrai duel à 2.</Text>
        </TouchableOpacity>
      </View></View>;
}
...
const s = StyleSheet.create({
  ...
  homeCreditText: { color: colors.success, fontSize: 10, fontWeight: '900' },
  ...
  battleSetupEyebrow: { color: colors.primaryLight, fontSize: 9, fontWeight: '900', letterSpacing: 1.2 },
  battleSetupTitle: { color: colors.textPrimary, fontSize: 14, fontWeight: '900', marginTop: 3 },
  battleSetupBadge: { color: colors.textMutedGrey, fontSize: 10, fontWeight: '800', paddingHorizontal: 9, paddingVertical: 5, borderRadius: 999, backgroundColor: colors.backgroundCard, borderWidth: 1, borderColor: colors.border },
  ...
  modeEyebrow: { color: '#DCD5F8', fontSize: 9, fontWeight: '900', letterSpacing: 1.1, marginBottom: 2 },
  ...
  headerCreditText: { color: colors.success, fontSize: 10, fontWeight: '900' },
  ...
  lobbySummaryLabel: { color: colors.textMutedGrey, fontSize: 8, fontWeight: '900', letterSpacing: .7, marginTop: 2, textAlign: 'center' },
  lobbySummaryDivider: { width: 1, height: 26, backgroundColor: colors.border },
  battleSelectionCopy: { flex: 1, minWidth: 0 },
  battleSelectionHint: { color: colors.textMutedGrey, fontSize: 9, lineHeight: 13, fontWeight: '700', marginTop: 3 },
  ...
  live: { marginTop: 14, padding: 7, borderRadius: 16, backgroundColor: '#100D14' }, liveHeader: { flexDirection: 'row', alignItems: 'center', gap: 6 }, dot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#6EE8A7' }, liveTitle: { color: '#FFF', fontSize: 12, fontWeight: '900' }, liveList: { gap: 6, paddingTop: 7 }, liveRowCompact: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 40, paddingHorizontal: 7, borderRadius: 14, backgroundColor: '#18131F' }, liveRowLeft: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, minWidth: 0 }, liveRowName: { flex: 1, color: '#FFF', fontSize: 12, fontWeight: '800', textDecorationLine: 'underline' }, avatarFallback: { backgroundColor: '#2B2235', alignItems: 'center', justifyContent: 'center' }, avatarLetter: { color: '#FFF', fontSize: 16, fontWeight: '900' }, username: { color: '#FFF', fontSize: 11, fontWeight: '800', marginTop: 3, maxWidth: 70 }, battleButton: { minHeight: 28, paddingHorizontal: 9, borderRadius: 14, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', marginTop: 4 }, battleButtonText: { color: '#FFFFFF', fontSize: 11, fontWeight: '900' }, battleButtonSending: { backgroundColor: '#8A7E4A', opacity: .85 }, battleButtonSent: { backgroundColor: '#1B1422', borderWidth: 1, borderColor: '#6EE8A7' }, battleButtonSentText: { color: '#6EE8A7' }, battleButtonBlocked: { backgroundColor: '#1B1422', borderWidth: 1, borderColor: '#FF5F83' }, battleButtonBlockedText: { color: '#FF5F83' }, invite: { marginTop: 10, minHeight: 132, paddingHorizontal: 16, paddingVertical: 15, borderRadius: 22, borderWidth: 1, borderColor: colors.primary, backgroundColor: colors.backgroundCard, justifyContent: 'center' }, inviteHead: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 }, inviteActions: { flexDirection: 'row', gap: 12, width: '100%' }, inviteLabel: { color: colors.primaryLight, fontSize: 15, lineHeight: 20, fontWeight: '900', marginTop: 4 }, inviteName: { color: '#FFF', fontSize: 17, lineHeight: 22, fontWeight: '900' }, inviteQuestion: { color: '#F3EDF7', fontSize: 16, lineHeight: 22, fontWeight: '800' }, inviteConnecting: { color: colors.primaryLight, fontSize: 13, lineHeight: 18, fontWeight: '900', textAlign: 'center', marginBottom: 8, letterSpacing: .5 }, no: { flex: 1, minHeight: 52, paddingHorizontal: 16, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.backgroundElevated, alignItems: 'center', justifyContent: 'center' }, noText: { color: '#FFF', fontSize: 16, fontWeight: '900' }, yes: { flex: 1, minHeight: 52, paddingHorizontal: 16, borderRadius: 18, borderWidth: 1, borderColor: colors.primaryLight, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' }, yesText: { color: '#FFFFFF', fontSize: 15, fontWeight: '900' }, actionDisabled: { opacity: .62 }, versus: { position: 'absolute', zIndex: 20, left: 16, right: 16, top: 120, padding: 16, borderRadius: 22, backgroundColor: colors.backgroundCard, borderWidth: 1, borderColor: colors.primary, alignItems: 'center' }, versusText: { color: colors.primaryLight, fontSize: 25, fontWeight: '900' }, versusNames: { color: '#FFF', fontSize: 12, fontWeight: '900', marginTop: 5 }, duel: { marginBottom: 6 }, duelNames: { flexDirection: 'row', alignItems: 'center' }, duelName: { color: '#FFF', fontSize: 13, fontWeight: '900' }, duelScore: { color: colors.primaryLight, fontSize: 15, fontWeight: '900' }, duelCenter: { minWidth: 46, alignItems: 'center', justifyContent: 'center' }, duelTimer: { color: '#FFF', fontSize: 11, fontWeight: '900', marginTop: 2 }, duelPoints: { color: '#FFF', fontSize: 13, fontWeight: '900', marginTop: 3 }, teamMembers: { flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginTop: 5 }, teamChip: { paddingHorizontal: 6, minHeight: 22, borderRadius: 11, backgroundColor: '#1D1625', alignItems: 'center', justifyContent: 'center' }, teamChipText: { color: '#FFF', fontSize: 11, fontWeight: '800' }, power: { height: 16, borderRadius: 8, overflow: 'hidden', backgroundColor: '#2A2032', flexDirection: 'row', position: 'relative', marginTop: 7 }, powerLeft: { height: '100%', backgroundColor: colors.primary }, powerRight: { flex: 1, height: '100%', backgroundColor: colors.success }, powerMiddle: { position: 'absolute', zIndex: 3, left: '50%', width: 2, height: '100%', backgroundColor: '#FFF' }, waiting: { padding: 16, borderRadius: 22, backgroundColor: colors.backgroundElevated, borderWidth: 1, borderColor: colors.border, alignItems: 'center' }, waitingPulse: { width: 54, height: 54, borderRadius: 27, backgroundColor: 'rgba(124,92,252,.16)', borderWidth: 1, borderColor: colors.primary, alignItems: 'center', justifyContent: 'center' }, waitingStatusPill: { marginTop: 10, minHeight: 28, paddingHorizontal: 12, borderRadius: 14, backgroundColor: 'rgba(124,92,252,.14)', borderWidth: 1, borderColor: colors.primary }, waitingStatusText: { color: colors.primaryLight, fontSize: 9, lineHeight: 26, fontWeight: '900', letterSpacing: .8 }, trophy: { fontSize: 34 }, winner: { color: '#FFF', fontSize: 19, fontWeight: '900', marginTop: 3 }, waitText: { color: '#FFF', fontSize: 11, lineHeight: 15, textAlign: 'center', marginTop: 6 }, browseText: { color: '#FFF', fontSize: 11, lineHeight: 16, marginBottom: 10 }, browseList: { gap: 7 }, browsePlayer: { flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.backgroundCard, paddingHorizontal: 10, paddingVertical: 10 }, browseNameRow: { flexDirection: 'row', alignItems: 'center', gap: 5 }, browseName: { color: colors.textPrimary, fontSize: 14, fontWeight: '900' }, browseChevron: { color: '#8F879D', fontSize: 16, fontWeight: '900' }, browseAvatarDot: { position: 'absolute', right: -1, bottom: -1 }, browseRankBadge: { color: colors.primaryLight, fontSize: 12, fontWeight: '900' }, browseMeta: { color: colors.success, fontSize: 10, fontWeight: '800', marginTop: 3 }, browseMetaShort: { color: '#FF5F83' }, browseBattle: { minHeight: 34, borderRadius: 17, backgroundColor: '#E5F266', paddingHorizontal: 10, alignItems: 'center', justifyContent: 'center' }, browseBattleText: { color: '#17130B', fontSize: 11, fontWeight: '900' }, shareButton: { minHeight: 40, borderRadius: 20, backgroundColor: colors.primary, paddingHorizontal: 16, alignItems: 'center', justifyContent: 'center', marginTop: 10 }, shareButtonText: { color: '#FFF', fontSize: 11, fontWeight: '900' },
