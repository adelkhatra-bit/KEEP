...
export default function KeepBattleMobileGameV2({enabled,onOpenProfile,onRequireAccount}:Props){
  ...
  React.useEffect(()=>{
    ...
    void tick();const id=setInterval(()=>void tick(),1000);
  ...
  const challenge=async(p:KeepBattleLivePlayer)=>{try{await sendBattleChallenge(p.profileId,solo?.themeCode||themeCode);Alert.alert('Défi envoyé',`@${p.username} a 15 secondes pour accepter.`);}catch{Alert.alert('Battle',`@${p.username} n’est plus disponible.`);void refreshSocial();}};
  ...
  if(solo){
    ...
    return <View style={s.root}>
      ...
      <View style={s.clockRow}><Text style={[s.clock,soloRemaining<1500&&s.clockHot]}>{(soloRemaining/1000).toFixed(1)}s</Text><Text style={s.clockHint}>RÉPONDS VITE</Text></View>
