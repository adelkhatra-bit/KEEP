...
type Props = { enabled: boolean; onOpenProfile: (username: string) => void; onRequireAccount?: () => void };
const ROUND_MS = 6000;
...
export default function KeepBattleMobileGameV2({enabled,onOpenProfile,onRequireAccount}:Props){
  ...
  const refreshSocial=React.useCallback(async()=>{
    ...
    try{
      ...
      if(accepted?.arenaId){await stopTrackPreview();await leaveSoloBattle().catch(()=>{});setSolo(null);setArena(await loadKeepBattleArena(accepted.arenaId));animateVersus();}
    }catch{}
  },[enabled,solo,animateVersus]);
  ...
  React.useEffect(()=>{
    ...
    void tick();const id=setInterval(()=>void tick(),4000);
  ...
  React.useEffect(()=>{if(!arena?.id)return undefined;const off=subscribeKeepBattleArena(arena.id,()=>void refreshArena());const id=setInterval(()=>void refreshArena(),350);return()=>{off();clearInterval(id);};},[arena?.id,refreshArena]);

  React.useEffect(()=>{
    const round=arena?.round;if(!arena||arena.status!=='ACTIVE'||!round?.previewUrl)return;
    const left=round.closesAt?Math.max(500,new Date(round.closesAt).getTime()-Date.now()):ROUND_MS;
    setAudioFailed(false);void autoplay(`arena:${arena.id}:${arena.matchNo}:${round.position}`,round.previewUrl,left);
  },[arena?.id,arena?.status,arena?.matchNo,arena?.round?.position,arena?.round?.previewUrl,autoplay]);
  ...
  React.useEffect(()=>{
    if(!arena||arena.status!=='WAITING'||!arena.isHost||arena.matchNo<=1||arena.seats.length<2)return undefined;
    const id=setTimeout(()=>void startKeepBattleArena(arena.id).then((a)=>{setArena(a);animateVersus();}).catch(()=>{}),4200);return()=>clearTimeout(id);
  },[arena?.id,arena?.status,arena?.isHost,arena?.matchNo,arena?.seats.length,animateVersus]);

  const startSolo=async()=>{if(busy)return;setBusy(true);try{const p=await loadKeepBattleSoloPack(themeCode,8);setArena(null);setSolo(p);setSoloIndex(0);setSoloAnswer(null);setSoloScore(0);setSoloStartedAt(Date.now());}catch(e:any){Alert.alert('KEEP Battle',String(e?.message||'Impossible de démarrer.'));}finally{setBusy(false);}};
  ...
  const answerArena=async(choice:string)=>{if(!arena||arena.status!=='ACTIVE'||arena.round?.answered||arena.round?.revealed||pending)return;void stopTrackPreview();setPending(choice);try{setArena(await submitKeepBattleArenaQuizAnswer(arena.id,choice));}catch{}finally{setPending(null);}};
  const challenge=async(p:KeepBattleLivePlayer)=>{try{await sendBattleChallenge(p.profileId,solo?.themeCode||themeCode);Alert.alert('Battle envoyé',`@${p.username} peut accepter maintenant.`);}catch{Alert.alert('Battle',`@${p.username} n’est plus disponible.`);void refreshSocial();}};
  ...
  if(solo){
    ...
    return <View style={s.root}>
      ...
      <View style={s.clockRow}><Text style={[s.clock,soloRemaining<2000&&s.clockHot]}>{(soloRemaining/1000).toFixed(1)}s</Text><Text style={s.clockHint}>RÉPONDS VITE</Text></View>
      ...
      <View style={s.scoreLine}><Text style={s.score}>{soloScore} bonne{soloScore>1?'s':''}</Text><Text style={s.score}>{8-soloIndex-1} restante{8-soloIndex-1>1?'s':''}</Text></View>
  ...
  if(arena){
    ...
    const left=r?.closesAt?Math.max(0,new Date(r.closesAt).getTime()-now):ROUND_MS;const pct=(left/ROUND_MS)*100;const first=players[0];const second=players[1];const total=Math.max(1,(first?.score||0)+(second?.score||0));const leftShare=players.length===2?Math.max(12,Math.min(88,((first?.score||0)/total)*100)):50;
    return <View style={s.root}>
      ...
      {first&&second?<View style={s.duel}><View style={s.duelNames}><Text style={s.duelName}>@{first.username}</Text><Text style={s.duelScore}>{first.score} — {second.score}</Text><Text style={[s.duelName,{textAlign:'right'}]}>@{second.username}</Text></View><View style={s.power}><View style={[s.powerLeft,{width:`${leftShare}%`}]} /><View style={s.powerMiddle}/><View style={s.powerRight}/></View></View>:null}
