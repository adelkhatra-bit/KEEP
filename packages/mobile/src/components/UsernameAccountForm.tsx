...
export default function UsernameAccountForm({ initialMode = 'create', followUsername = '', onSuccess }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (followUsername) {
        ...
        Alert.alert('Compte KEEP créé', 'Ton profil d’essai, tes crédits, ton pseudo et ton mot de passe sont maintenant rattachés au même compte KEEP.');
      }
      onSuccess?.();
    } catch {
  ...
  >
    ...
    <Text style={s.subtitle}>
      {mode === 'create'
        ? 'Simple : choisis ton pseudo KEEP et ton mot de passe. Aucun e-mail, aucun code à attendre.'
        : 'Saisis ton pseudo KEEP et ton mot de passe.'}
    </Text>
    ...
    <TextInput
      ...
      maxLength={30}
    />
    <Text style={s.usernameHint}>{mode === 'create' ? 'Ton pseudo est public et unique sur KEEP.' : 'Exemple : adel4A'}</Text>
