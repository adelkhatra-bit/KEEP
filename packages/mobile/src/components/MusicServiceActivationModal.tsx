        ...
        <View style={s.card} accessibilityViewIsModal>
          ...
          <View style={s.hero}>
            ...
            <View style={s.heroText}>
              <Text style={s.eyebrow}>SERVICE MUSICAL Loki Music</Text>
              <Text style={s.title}>Activer {name}</Text>
              <Text style={s.subtitle}>{supportsAccountSync ? `Tu gardes le contrôle. Après l’activation Loki Music, ${name} te demandera directement l’autorisation de connecter ton compte.` : 'Tu gardes le contrôle. Loki Music mémorise uniquement que tu veux utiliser ce service.'}</Text>
            </View>
          </View>

          <View style={s.infoBox}>
            <Text style={s.infoTitle}>Comment ça va fonctionner</Text>
            <Step n="1" text={`Loki Music réserve 1 emplacement de ta formule ${planLabel}.`} />
            <Step n="2" text="Ce choix est sauvegardé dans ton compte Loki Music et reste présent après fermeture ou changement d’appareil." />
            <Step n="3" text={supportsAccountSync ? `Tu autorises ensuite ${name} avec sa page officielle. Loki Music pourra importer uniquement les métadonnées autorisées de tes favoris/playlists et synchroniser tes Vibes vers tes playlists.` : `Quand tu envoies un morceau ou une Vibe vers ${name}, Loki Music prépare la musique puis ouvre le service choisi.`} />
          </View>

          <View style={s.safeBox}>
            <Text style={s.safeTitle}>✓ Tes accès restent privés</Text>
            <Text style={s.safeText}>{supportsAccountSync ? `Loki Music ne reçoit jamais ton mot de passe ${name}. La connexion passe par OAuth officiel. Loki Music stocke le jeton serveur nécessaire et les métadonnées que tu autorises ; aucun fichier audio protégé n’est téléchargé, copié ou ré-uploadé.` : `Loki Music ne demande pas ton mot de passe ${name} et ne publie rien à ta place. Sans connexion API du fournisseur, Loki Music utilise simplement le passage sécurisé vers l’application ou le site du service.`}</Text>
          </View>
