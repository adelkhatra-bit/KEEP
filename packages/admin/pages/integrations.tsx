...
const FIELDS: SecretField[] = [
  { key: 'BREVO_API_KEY', label: 'Brevo API transactionnelle', category: 'E-mails', help: 'Clé API Brevo utilisée pour l’envoi HTTP.' },
  { key: 'BREVO_SMTP_KEY', label: 'Brevo SMTP key', category: 'E-mails', help: 'Mot de passe SMTP Brevo. Toujours masqué.' },
...
export default function IntegrationsPage() {
  ...
  const saveField = async (field: SecretField) => {
    ...
    // Le Super Admin preview est statique. En production, cette route est
    // appelée avec une session KEEP Super Admin réelle et le secret est
    // chiffré côté backend avant stockage dans Supabase.
  ...
  return (
    <AdminLayout>
      ...
      <div className="demo-banner" style={{ marginBottom: 24 }}>
        🔐 Les secrets sont masqués. Ils ne doivent jamais être commités dans GitHub ni renvoyés en clair au navigateur. En production, ils sont chiffrés côté backend avant stockage.
      </div>
              ...
              <div key={field.key} className="kpi-card" style={{ display: 'grid', gridTemplateColumns: 'minmax(210px, 1fr) minmax(260px, 1.4fr) auto', gap: 16, alignItems: 'center' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    placeholder={saved[field.key] ? '••••••••••••  configuré' : 'Saisir une nouvelle valeur'}
                    autoComplete="off"
                ...
                <button
                  ...
                  style={{ minWidth: 110, padding: '11px 14px', borderRadius: 999, border: 0, cursor: values[field.key]?.trim() ? 'pointer' : 'not-allowed', background: values[field.key]?.trim() ? 'var(--primary)' : 'var(--border)', color: '#fff', fontWeight: 800 }}
                >
                  {saved[field.key] ? 'Remplacer' : 'Enregistrer'}
                </button>
              </div>
            ))}
          </div>
