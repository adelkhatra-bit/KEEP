...
for /L %%I in (1,1,90) do (
  powershell -NoProfile -Command "try { $r=Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:8081/' -TimeoutSec 2; if($r.StatusCode -ge 200 -and $r.StatusCode -lt 500){exit 0}else{exit 1} } catch { exit 1 }" >nul 2>&1
  if not errorlevel 1 (
    set "READY=1"
    goto :mobile_ready
  )
...
if not defined READY (
  ...
  echo Ne pas ouvrir une page blanche. Verifie la fenetre KEEP MOBILE.
  pause
  exit /b 1
)
